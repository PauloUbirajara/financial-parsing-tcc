<script lang="ts">
  ...
  const breadcrumbs = [
    { label: "Carteiras", href: "/app/wallets" },
    { label: wallet.name, href: `/app/wallets/${wallet.id}` },
    { label: "Editar", href: `/app/wallets/${wallet.id}/edit` },
  ];

  async function onUpdate() {
    const id = $page.params.id;
    const response = await fetch(`/app/wallets/${id}/edit`, {
      method: "POST",
