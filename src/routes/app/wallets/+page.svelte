<script lang="ts">
  ...
  async function onDelete() {
    ...
    try {
      const response = await fetch(
        `/app/wallets/${selectedModel?.id}?/delete`,
        {
          method: "POST",
          body: JSON.stringify({}),
        },
  ...
  const breadcrumbs = [{ label: "Carteiras", href: "/app/wallets" }];

  async function onBulkDelete(ids: string[]) {
    const response = await fetch("/app/wallets?/bulk-delete", {
      method: "POST",
  ...
  async function updateResults(search: string) {
    ...
    let url = `/app/wallets?${urlParams.toString()}`;
