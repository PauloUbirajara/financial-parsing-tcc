<script lang="ts">
  ...
  const breadcrumbs = [
    ...
    { label: transaction.name, href: `/app/transactions/${transaction.id}` },
    { label: "Editar", href: `/app/transactions/${transaction.id}/edit` },
  ];

  async function onUpdate() {
    const id = $page.params.id;
    const response = await fetch(`/app/transactions/${id}/edit`, {
      method: "POST",
