<script lang="ts">
  ...
  function onEdit() {
    goto(`/app/transactions/${transaction.id}/edit`);
  }
  ...
  async function onDelete() {
    const response = await fetch(
      `/app/transactions/${transaction.id}?/delete`,
      {
        method: "POST",
        body: JSON.stringify({}),
      },
  ...
  <div class="content">
    ...
    <div class="flex items-center gap-4">
      <A
        ...
        href={`/app/wallets/${transaction.wallet.id}`}
      >
  ...
  <div class="content">
    ...
    <div class="flex flex-wrap p-4 bg-white rounded gap-2">
      {#each transaction.categories as item}
        <Badge color="primary" href={`/app/categories/${item.id}`} large>
          {item.name}
        </Badge>
      {/each}
    </div>
