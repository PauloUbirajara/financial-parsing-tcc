<script lang="ts">
  ...
  async function onDelete() {
    ...
    try {
      const response = await fetch(
        `/app/transactions/${selectedModel?.id}?/delete`,
        {
          method: "POST",
          body: JSON.stringify({}),
        },
  ...
  const breadcrumbs = [{ label: "Transações", href: "/app/transactions" }];

  async function onBulkDelete(ids: string[]) {
    const response = await fetch("/app/transactions?/bulk-delete", {
      method: "POST",
  ...
  async function updateResults(search: string) {
    ...
    let url = `/app/transactions?${urlParams.toString()}`;
