<script lang="ts">
  ...
  async function onDelete() {
    ...
    try {
      const response = await fetch(
        `/app/categories/${selectedModel?.id}?/delete`,
        {
          method: "POST",
          body: JSON.stringify({}),
        },
  ...
  const breadcrumbs = [{ label: "Categorias", href: "/app/categories" }];

  async function onBulkDelete(ids: string[]) {
    const response = await fetch("/app/categories?/bulk-delete", {
      method: "POST",
  ...
  async function updateResults(search: string) {
    ...
    let url = `/app/categories?${urlParams.toString()}`;
