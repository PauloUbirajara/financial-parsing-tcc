...
from apps.user_activation.serializers import UserActivationRegisterSerializer
...
class UserActivationRegisterView(APIView):
    def post(self, request):
        ...
        try:
            ...
            existing_user_activation = (
                UserActivation.objects
                .filter(user=created)
                .first()
            )
            if existing_user_activation is not None:
                return Response(status=HTTPStatus.CONFLICT)

            user_activation = UserActivation.objects.create(user=created)
            ...
            send_mail(
                ...
                message="Use this link to active your account on the Financial Parsing platform: {}".format(activation_link),
                from_email=None,
                recipient_list=[created.email],
                fail_silently=False,
            )
